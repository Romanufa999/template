...
- При деплое CMS: `romanadmin.html` деплоится на S3 рядом с сайтом, а в HTML-файлы сайта встраивается inline-скрипт и пересобирается/передеплоится
- **Суть разделения для безопасности:** сайт работает без CMS. CMS — это надстройка, которая ничего не ломает если её убрать. Без credentials администратора сайт остаётся чистой статикой без каких-либо обращений к Supabase.
...
**В исходном коде страницы НЕТ данных Supabase.** Ни URL базы, ни anon key. Адрес базы известен только во время сборки, но НЕ зашивается в HTML. В коде есть только имя сайта (идентификатор схемы вроде `stroymsk1`) — оно само по себе бесполезно без URL базы и ключа.

**Без ввода credentials сайт полностью статический:**
- Никаких fetch-запросов к Supabase
- Нет кнопок редактирования, рамок, подсветки
- CSS-стили и DOM-элементы редактора НЕ создаются
- Публичные данные загружены в HTML при сборке (prebuild), не через клиентский fetch
- В исходном коде — только кнопка-замочек и минимальный JS для показа формы входа

Inline-редактор начинает работать **только** когда админ вводит адрес базы + API ключ + пароль и Supabase подтверждает авторизацию.

#### Поток авторизации

```
1. В cookie есть cms_url + cms_key + cms_pwd?
   ├─ НЕТ → кнопка-замочек, по клику форма с тремя полями
...
1. **Замок (не залогинен)** — нет cookie или пароль не прошёл проверку. По клику → модальное окно с **тремя полями**:
   - **Адрес базы** (URL Supabase, например `https://xxxxx.supabase.co`)
   - **API ключ** (anon key Supabase)
   - **Пароль**
   После успешной проверки через Supabase → все три значения в cookie (max-age 30 дней):
   - `cms_url` — адрес базы
   - `cms_key` — API ключ
   - `cms_pwd` — пароль
...
**Когда пересобирать:** после того как администратор изменил контент через админку или inline-редактор, нужна пересборка и передеплой чтобы изменения попали в статику. Inline-редактор обновляет данные в Supabase, но визуальные изменения на странице — только для текущей сессии администратора (через JS). Для посетителей изменения появятся после пересборки.

### 10. Встроить inline-редактор

Добавить перед `</body>` каждой HTML-страницы скрипт inline-редактирования (см. "Режим 2: Inline-редактирование"). В коде — только имя сайта и логика замочка/формы входа. Никаких URL и ключей.

### 11. Деплой и credentials
...
### 12. Зафиксировать привязку CMS в CLAUDE.md проекта
...
9. **CMS и сайт — раздельные процессы.** Сайт деплоится независимо от CMS. CMS — надстройка, которая не влияет на работу сайта. Без credentials администратора сайт = чистая статика, ноль обращений к Supabase.
