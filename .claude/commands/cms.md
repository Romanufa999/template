...
После выполнения навыка должна получиться система, в которой:

1. **Админка (`romanadmin.html`)** — единственный интерфейс для редактирования контента. Живёт на S3 рядом с сайтом. На самих страницах сайта нет никакого CMS-кода.
2. **Данные в Supabase** — контент хранится в схеме `cms_{SITE}` (секции, поля, коллекции, файлы). Админ меняет данные через админку.
3. **Сборка при деплое** — при сборке/пересборке сайта Claude читает контент из Supabase через публичные RPC и **вписывает прямо в HTML**. Результат — чистая статика без runtime-запросов к Supabase.
4. **Цикл обновления:** админ поменял контент в админке → запросил пересборку → Claude подтягивает данные из Supabase → генерирует HTML → деплоит на S3.

**Для посетителя сайт = чистая статика.** Никаких обращений к Supabase, никакого JS для загрузки контента. Всё вшито в HTML при сборке.

## CMS и сайт — независимые процессы
...
2. **CMS** — схема в Supabase + админка (`romanadmin.html`). Создаётся **отдельно**, деплоится **отдельно**.
